...
**当前进度：** 下列生命周期、作用域、安全、检索、WorkingSet 注入和 HTTP/原生工具接口已在 `v0.2.3` 完成。GUI 已能搜索、查看召回解释与审计、确认/替换候选、固定/取消固定、单条纠正、删除和导出带审计的 JSON；批量纠错仍待实现。纠正不原地改写旧值，而是生成用户证据事件并显式 supersede。
