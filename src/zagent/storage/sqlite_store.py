...
class SqliteStore:
    ...
    def create_session(self, title: str = "新任务") -> Dict[str, Any]:
        session_id = "ses_" + uuid.uuid4().hex
        now = self._now()
        with self._lock, self._db:
            self._db.execute(
                "INSERT INTO sessions(session_id,title,created_at,updated_at) VALUES(?,?,?,?)",
                (session_id, title.strip() or "新任务", now, now),
            )
    ...
    def search_events(self, session_id: str, query: str, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        rows = self._db.execute(
            ...
            (expression, session_id, max(1, min(limit, 100))),
        ).fetchall()
        normalized_query = query.casefold()
        results: List[Dict[str, Any]] = []
        for row in rows:
            event = self._to_event(row)
            serialized = self._serialize(event.payload)
            exact_boost = 1.0 if normalized_query in serialized.casefold() else 0.0
            results.append({"event": event.to_dict(), "excerpt": excerpt(serialized),
                            "score": float(row["score"]) - exact_boost})
    ...
    def create_archive(
        ...
        summary_event = self.append_event(session_id, "archive", "system", summary, tags=["archive"])
        with self._db:
            self._db.execute(
